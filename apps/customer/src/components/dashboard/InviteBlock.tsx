...
import { Copy, Link2, Mail, QrCode, Share, X } from "lucide-react";
...
export function InviteBlock({ userId, kind, familyId, label, createInvite, onSent }: InviteBlockProps) {
  ...
  const ensureLink = useCallback(async (): Promise<string | null> => {
    ...
    try {
      ...
      toast.error(err instanceof Error ? err.message : "Could not create the invite link");
      return null;
    } finally {
      setGenerating(false);
    }
  }, [link, userId, createInvite, onSent]);
